...
export function getPinButton(row: Row, text: string, data: string): string {
...
export type SidebarSection<T extends string> = {
    ...
    sectionKey: string;
    items: readonly T[];
    checkedItemsGetter: () => Set<T>;
    itemVerifyFn: (v: unknown) => boolean;
};
...
export function getSidebarHtml<T extends string>(sections: SidebarSection<T> | SidebarSection<T>[]): string {
    ...
    for (const section of sections) {
        ...
        for (const item of section.items) {
            ...
            parts.push(
                `<label><input type="checkbox"${checked} data-${section.sectionKey}="${escaped}">${escaped}</label>`
            );
        }
