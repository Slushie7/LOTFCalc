...
import { type Weapon, type WeaponClass, isWeaponClass, type CalculatedWeaponStats } from '../model.js';
...
function isSettingToggleKey(k: unknown): k is BooleanKeys<WeaponsState> {
...
function isGroupToggleKey(k: unknown): k is WeaponsSuperheaderKey {
    return typeof k === 'string' && Object.hasOwn(GroupToggles.toggles, k);
}

const sortFunctions: Record<WeaponsHeaderKey, SortFunction<CalculatedWeaponStats>> = {
    // INFO
...
class WeaponsView extends View {
    ...
    private onTableBodyClick(e: Event): void {
        ...
        if (this.state.pinnedItems.has(weaponKey)) this.state.pinnedItems.delete(weaponKey);
        else this.state.pinnedItems.add(weaponKey);
    ...
    private renderWeaponsModeElements(): void {
        if (!this.isActiveMode()) return;
    ...
    private renderWeapons(weaponFadeIn: string | null = null): void {
        ...
        const rows = calcStats.map((cs) =>
            getWeaponRow(cs, this.state.showColGroups, this.state.showSplit, this.state.showRawScaling)
        );
