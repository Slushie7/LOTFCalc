...
import type { HeaderGroup } from './header.js';
import type { DamageSplit, CalculatedWeaponStats, HeaderKey, SuperheaderKey } from './model.js';
 ...
 * @returns
 */
export function getHeaderHtml(groups: readonly HeaderGroup[], sortKey: HeaderKey, ascending: boolean): string {
    ...
    groups.forEach((group, superIdx) => {
        ...
        group.columns.forEach((col, idx) => {
            ...
            if (idx === 0) classes.push('col-starter');
...
export function getWeaponsHtml(weaponRows: readonly WeaponRow[]): string {
    ...
    for (const row of weaponRows) {
        ...
        row.cells.forEach((cell, idx) => {
            if (idx === 0) {
                ...
                rowParts.push(
                    `<td class="${cell.cls}"><a class="${cell.cls}" href="${escapeHtml(url)}" target="_blank" rel="noopener noreferrer">${escapeHtml(cell.text)}</a></td>`
                );
            } else {
...
const sortFunctions: Record<HeaderKey, SortFunction> = {
    // INFO
...
export function sortCalculated(calculated: CalculatedWeaponStats[], sortKey: HeaderKey, ascending: boolean): void {
    const fn = sortFunctions[sortKey];
    if (fn !== undefined) {
        if (ascending) calculated.sort(fn);
        else calculated.sort((a, b) => -fn(a, b));
    }
}
