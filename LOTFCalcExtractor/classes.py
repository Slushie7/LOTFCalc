...
class Curve:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {'key': self.key, '_interp_mode': self._interp_mode, '_points': self._points}
...
class StatScalarGradeRange:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {'grade': self.grade, 'min_incl': self.min_incl, 'max_excl': self.max_excl}
...
class StatScaledDamage:
    ...
    def calculate_damage_contribution(
        ...
        player_scalar_stat = self.stat_curve.interpolate(player_stat_level) if self.stat_curve else 0.0
        ...
        additional_ar_physical = (
            self.base_damage.dmg_physical.get_value(upgrade_level) * factor if self.stat in ('S', 'A') else 0.0
        )
        additional_ar_holy = (
            self.base_damage.dmg_holy.get_value(upgrade_level) * factor if self.stat in ('R', 'I') else 0.0
        )
        additional_ar_fire = (
            self.base_damage.dmg_fire.get_value(upgrade_level) * factor if self.stat in ('R', 'I') else 0.0
        )
        additional_ar_wither = (
            self.base_damage.dmg_wither.get_value(upgrade_level) * factor if self.stat in ('R', 'I') else 0.0
        )
        additional_spellpower = (
            self.base_damage.dmg_spell.get_value(upgrade_level) * factor if self.stat in ('R', 'I') else 0.0
        )

        return AttackRating(
            additional_ar_physical, additional_ar_holy, additional_ar_fire, additional_ar_wither, additional_spellpower
        ), weapon_scaling_stat
    ...
    @classmethod
    def from_dict(cls, d: dict[str, Any], curves_d: dict[str, Curve], base_damages_d: dict[str, BaseDamage]) -> Self:
...
class WeaponRuneSockets:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {'rune_sockets': self.rune_sockets, 'curve_key': self.num_by_level.key if self.num_by_level else None}
...
class WeaponDamageAR:
    ...
    def calculate(
        ...
        add_ar_str, scaling_str = self.scaled_str.calculate_damage_contribution(upgrade_level, player_stats.strength)
        add_ar_agi, scaling_agi = self.scaled_agi.calculate_damage_contribution(upgrade_level, player_stats.agility)
        add_ar_rad, scaling_rad = self.scaled_rad.calculate_damage_contribution(upgrade_level, player_stats.radiance)
        add_ar_inf, scaling_inf = self.scaled_inf.calculate_damage_contribution(upgrade_level, player_stats.inferno)

        # calculate how much is added to each damage type from all stats combined
        add_ar_physical = add_ar_str.physical + add_ar_agi.physical + add_ar_rad.physical + add_ar_inf.physical
        add_ar_holy = add_ar_str.holy + add_ar_agi.holy + add_ar_rad.holy + add_ar_inf.holy
        add_ar_fire = add_ar_str.fire + add_ar_agi.fire + add_ar_rad.fire + add_ar_inf.fire
        ...
        return (
            ...
            CalculatedWeaponScaling(scaling_str, scaling_agi, scaling_rad, scaling_inf, stat_grade_ranges),
            wieldable,
        )
...
class CalculatedWeaponAR:
    ...
    def __post_init__(self) -> None:
        object.__setattr__(
            self, 'total_dmg', self.physical.total + self.holy.total + self.fire.total + self.wither.total
        )
